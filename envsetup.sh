...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^styx_") ; then
        STYX_BUILD=$(echo -n $1 | sed -e 's/^styx_//g')
    else
        STYX_BUILD=
    fi
    export STYX_BUILD
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the Styx Project GitHub
        T=$(gettop)
        cd $T > /dev/null
...
function repopick() {
    T=$(gettop)
    $T/vendor/styx/build/tools/repopick.py $@
}
